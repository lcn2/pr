...
main(int argc, char *argv[])
{
    ...
    relpath = calloc_path(".", __FILE__);
    if (relpath != NULL) {
        errno = 0;  /* pre-clear errno for errp() */
        if (stat(relpath, &in_st) != 0) {
            errp(126, __func__, "couldn't stat file %s", relpath);
            not_reached();
        }
        bytes = copyfile(relpath, "pr_test.copy.c", true, 0);
        fdbg(stderr, DBG_MED, "copyfile(\"%s\", \"pr_test.copy.c\", true, 0): %ju bytes", relpath, (uintmax_t)bytes);
        errno = 0; /* pre-clear errno for errp() */
        if (stat("pr_test.copy.c", &out_st) != 0) {
            errp(128, __func__, "couldn't stat file pr_test.copy.c");
            not_reached();
        }

        if (!is_mode("pr_test.copy.c", in_st.st_mode)) {
            err(129, __func__, "copyfile() failed to copy st_mode of file %s", relpath);
            not_reached();
        } else {
         ...
         */
        errno = 0;      /* pre-clear errno for errp() */
        if (unlink("pr_test.copy.c") != 0) {
            errp(130, __func__, "unable to delete file pr_test.copy.c");
            not_reached();
        } else {
         ...
         */
        if (!is_empty("pr_test.copy.c")) {
            errp(131, __func__, "just deleted file pr_test.copy.c is not empty");
            not_reached();
        }
    ...
    relpath = calloc_path(".", __FILE__);
    if (relpath != NULL) {
        bytes = copyfile(relpath, "pr_test.copy.c", false, S_IRUSR|S_IWUSR);
        fdbg(stderr, DBG_MED, "copyfile(\"%s\", \"pr_test.copy.c\", false, S_IRUSR|S_IWUSR): %ju bytes", relpath,
                (uintmax_t)bytes);
        ...
        errno = 0;  /* pre-clear errno for errp() */
        if (stat("pr_test.copy.c", &out_st) != 0) {
            errp(132, __func__, "failed to stat pr_test.copy.c");
            not_reached();
        }

        if ((out_st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) != (S_IRUSR|S_IWUSR)) {
            err(133, __func__, "copyfile() failed to set S_IRUSR|S_IWUSR on pr_test.copy.c");
            not_reached();
        } else {
            fdbg(stderr, DBG_MED, "copyfile() successfully set S_IRUSR|S_IWUSR on pr_test.copy.c");
        }
        /*
         * delete copied file
         */
        errno = 0;      /* pre-clear errno for errp() */
        if (unlink("pr_test.copy.c") != 0) {
            errp(134, __func__, "unable to delete file pr_test.copy.c");
            not_reached();
        } else {
            fdbg(stderr, DBG_MED, "successfully deleted copied file pr_test.copy.c");
        }
         ...
         */
        if (!is_empty("pr_test.copy.c")) {
            errp(135, __func__, "just deleted file pr_test.copy.c is not empty");
            not_reached();
        }
    ...
    if (stat("pr.o", &in_st) != 0) {
        errp(136, __func__, "failed to stat pr.o");
        not_reached();
    }
    /*
    ...
    if (stat("pr.copy.o", &out_st) != 0) {
        errp(137, __func__, "failed to stat pr.copy.o");
        not_reached();
    }

    if (!is_mode("pr.copy.o", in_st.st_mode)) {
        err(138, __func__, "pr.o st_mode != pr.copy.o st_mode: %04o != %04o",
                in_st.st_mode & ~S_IFMT, out_st.st_mode & ~S_IFMT);
        not_reached();
    } else {
    ...
    if (size1 <= 0 || size1 != size2 || size1 != in_st.st_size || size1 != out_st.st_size) {
        err(139, __func__, "size_if_file(\"pr.o\") != size_if_file(\"pr.copy.o\") or <= or != stat(2) st_size!");
        not_reached();
    } else {
    ...
    if (unlink("pr.copy.o") != 0) {
        errp(140, __func__, "unable to delete file pr.copy.o");
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "successfully deleted copied file pr_test.copy.c");
    }
    ...
    if (!is_empty("pr.copy.o")) {
        errp(141, __func__, "just deleted file pr.copy.o is not empty");
        not_reached();
    }
    ...
    if (!exists(relpath)) {
        err(142, __func__, "%s does not exist", relpath);
        not_reached();
    } else if (!is_dir(relpath)) {
        err(143, __func__, "%s is not a directory", relpath);
        not_reached();
    } else {
    ...
    if (!dir_exists) {
        if (!is_mode(relpath, 0755)) {
            err(144, __func__, "failed to set mode of %s to %o", relpath, 0755);
            not_reached();
        } else {
    ...
    mkdirs(-1, relpath, 0755);
    if (!exists(relpath)) {
        err(145, __func__, "%s does not exist", relpath);
        not_reached();
    } else if (!is_dir(relpath)) {
        err(146, __func__, "%s is not a directory", relpath);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "%s is a directory", relpath);
    }
    ...
    if (!dir_exists) {
        if (!is_mode(relpath, 0755)) {
            err(147, __func__, "failed to set mode of %s to %o", relpath, 0755);
            not_reached();
        } else {
            fdbg(stderr, DBG_MED, "%s is mode 07555", relpath);
        }
        ...
        if (!has_mode(relpath, 0755)){
            err(148, __func__, "%s does not have bits %o set", relpath, 0755);
            not_reached();
        } else {
        ...
        if (!has_mode(relpath, S_IRWXU)) {
            err(149, __func__, "%s does not have bits %o set", relpath, S_IRWXU);
            not_reached();
        } else {
        ...
        if (has_mode(relpath, S_ISUID)) {
            err(150, __func__, "%s has set user id on execution", relpath);
            not_reached();
        } else {
    ...
    if (ent == NULL) {
        err(151, __func__, "read_fts() returned a NULL pointer on \"\"");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
            if (p == NULL) {
                err(152, __func__, "fts_path(ent) returned NULL");
                not_reached();
            }
            switch (ent->fts_info) {
                case FTS_F:
                    ...
                    if (is_file(p)) {
                        ...
                        err(153, __func__, "%s reported as FTS_FILE !is_file(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_D:
                    ...
                    if (is_dir(p)) {
                        ...
                        err(154, __func__, "%s reported as FTS_D !is_dir(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_SL:
                    ...
                    if (is_symlink(p)) {
                        ...
                        err(155, __func__, "%s reported as FTS_SL !is_symlink(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_SLNONE:
                    ...
                    break;
                case FTS_NS:
                    err(156, __func__, "no stat(2) info available for path %s in tree", p);
                    not_reached();
                    break;
                case FTS_NSOK:
    ...
    ent = read_fts("", -1, &cwd, &fts);
    if (ent == NULL) {
        err(158, __func__, "read_fts() returned a NULL pointer on \"\" for directories");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
            if (p == NULL) {
                err(159, __func__, "fts_path(ent) returned NULL");
                not_reached();
            }
            if (ent->fts_info != FTS_D && ent->fts_info != FTS_DP) {
                err(160, __func__, "%s is not a directory", p);
                not_reached();
            } else {
    ...
    if (ent == NULL) {
        ...
        do {
            char *p = fts_path(ent);
            if (p == NULL) {
                err(162, __func__, "fts_path(ent) returned NULL");
                not_reached();
            }
            if (ent->fts_info != FTS_F) {
                err(163, __func__, "%s is not a file", p);
                not_reached();
            } else if (!strncmp(p, "pr",4)) {
                err(164, __func__, "found file meant to be ignored: %s", p);
                not_reached();
            } else {
    ...
    ent = read_fts("", -1, &cwd, &fts);
    if (ent == NULL) {
        err(169, __func__, "read_fts() returned a NULL pointer on \"\"");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
            if (ent == NULL) {
                err(170, __func__, "fts_path(ent) returned NULL");
                not_reached();
            }
            switch (ent->fts_info) {
                case FTS_F:
                ...
                case FTS_SLNONE:
                    ...
                    break;
                case FTS_NS:
                    err(171, __func__, "no stat(2) info available");
                    not_reached();
                    break;
                case FTS_NSOK:
                    err(172, __func__, "encountered FTS_NSOK for path %s in tree: FTS_NOSTAT set!", p);
                    not_reached();
                    break;
            }
        } while ((ent = read_fts(NULL, -1, &cwd, &fts)) != NULL);
    ...
    if (paths_found != NULL) {
        err(176, __func__, "found paths when none should have been found");
        not_reached();
    }
    ...
    if (name == NULL) {
         ...
         * "pr_test" must exist
         */
        err(178, __func__, "didn't find \"pr_test\" in paths array");
        not_reached();
    } else {
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(183, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    paths_found = find_paths(paths, NULL, -1, &cwd, false, &fts);
    if (paths_found == NULL) {
        err(184, __func__, "didn't find any files in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(185, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    paths_found = find_paths(paths, NULL, -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(190, __func__, "found paths when none should have been found");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, NULL, -1, &cwd, false, &fts);
    if (paths_found == NULL) {
        err(191, __func__, "unable to find any path in the directory");
        not_reached();
    }
    ...
    if (paths_found == NULL) {
        err(192, __func__, "didn't find any paths in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        ...
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(193, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found != NULL) {
        err(201, __func__, "found unexpected socket under cwd");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(202, __func__, "found unexpected character device under cwd");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(203, __func__, "found unexpected block device under cwd");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(204, __func__, "found unexpected FIFO under cwd");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        ...
        for (j = 0; j < len; ++j) {
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {
                err(205, __func__, "found NULL pointer in paths_found (not file, directory or symlink) array");
                not_reached();
            }
         ...
         * make it an error
         */
        err(206, __func__, "found unexpected file type under cwd");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        ...
        for (j = 0; j < len; ++j) {
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {
                err(208, __func__, "found NULL pointer in paths_found (any) array");
                not_reached();
            }
            if (strcmp(relpath, name) != 0) {
                err(209, __func__, "found non-matching file in list: %s != %s", name, relpath);
                not_reached();
            }
            fdbg(stderr, DBG_MED, "found file %s as case-sensitive search", name);
        }
    } else {
        err(210, __func__, "couldn't find any file called \"%s\" as case-sensitive search", relpath);
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        ...
        for (j = 0; j < len; ++j) {
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {
                err(211, __func__, "found NULL pointer in paths_found (any) array");
                not_reached();
            }
            if (strcasecmp(relpath, name) != 0) {
                err(212, __func__, "found non-matching file in list: %s != %s", name, relpath);
                not_reached();
            }
    ...
    if (unlink(relpath) != 0) {
        if (errno != ENOENT) {
            errp(214, __func__, "unable to delete file %s", relpath);
            not_reached();
        }
    } else {
     ...
     */
    if (!is_empty(relpath)) {
        errp(215, __func__, "just deleted file %s is not empty", relpath);
        not_reached();
    }
     ...
     */
    if (!exists(relpath)) {
        err(216, __func__, "file %s does not exist", relpath);
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        ...
        for (j = 0; j < len; ++j) {
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {
                err(218, __func__, "found NULL pointer in paths_found array");
                not_reached();
            }
            if (strcasecmp(relpath, name) != 0) {
                err(219, __func__, "found non-matching file: %s != %s", name, relpath);
                not_reached();
            }
            fdbg(stderr, DBG_MED, "found %s by case-insensitive search", name);
        }
         ...
         */
        err(220, __func__, "couldn't find any file called %s by case-insensitive search", relpath);
        not_reached();
    }
    ...
    if (fname != NULL) {
        ...
        err(221, __func__, "couldn't find %s in tree", relpath);
        not_reached();
    }
    ...
    if (unlink(relpath) != 0) {
        if (errno != ENOENT) {
            errp(222, __func__, "unable to delete file %s", relpath);
            not_reached();
        }
    } else {
        fdbg(stderr, DBG_MED, "successfully deleted created file %s", relpath);
     ...
     */
    if (!is_empty(relpath)) {
        errp(223, __func__, "just deleted file %s is not empty", relpath);
        not_reached();
    }
